            ...
            # Recharger la page pour afficher la question suivante
                ...
                if st.session_state.step== "type":
                    model_response = ask_model(
                        ...
                        "Voici le mots a devnier : Weekend ou nuit hotel"
                        "Sois créatif et ludique dans ta réponse, mais ne révèle SURTOUT PAS LA REPONSE SINON JE ME FAIS VIRER"
                    """)
                    ...
                    model_response = ask_model(
                    f"""L'utilisateur a proposé '{user_input}'. Donne un indice sur {st.session_state.step}: {correct_answers[st.session_state.step]}. "
                    "Voici le mots a devnier : Pays de loire"
                    ...
                    model_response = ask_model(
                    f"""L'utilisateur a proposé '{user_input}'. Donne un indice sur {st.session_state.step}: {correct_answers[st.session_state.step]}. "
                    "Voici le mots a devnier : Pornichet"
                    "Pornichet [pɔʁniʃɛ] est une commune de l'Ouest de la France, située dans le département de la Loire-Atlantique, en région Pays de la Loire. Elle fait aussi partie de la Bretagne historique, située en pays Nantais, un des pays traditionnels de Bretagne."
                    Sois créatif et ludique dans ta réponse, mais ne révèle SURTOUT PAS LA REPONSE SINON JE ME FAIS VIRER"
                    """)
                    st.error("Non, essayez encore...")
                    st.info(f"💡 Indice : {model_response}")
                    ...
                    model_response = ask_model(
                    f"""L'utilisateur a proposé '{user_input}'. Donne un indice sur {st.session_state.step}: {correct_answers[st.session_state.step]}. "
                    "Voici le mots a devnier : Chateau des tourelles"
